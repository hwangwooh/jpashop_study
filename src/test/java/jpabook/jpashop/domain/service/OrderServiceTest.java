...
public class OrderServiceTest {
    ...
    public void 상품주문() throws Exception {
        ...
        member.setName("회원1");
        member.setAddress(new Address("서울","어디가","123456"));
        em.persist(member);
        Book book = new Book();
        book.setName("jpa책");
        book.setPrice(20000);
        ...
        Assert.assertEquals("주문한 상품 종류 수가 정확애야한다", 1,getOrder.getOrderItems().size());
        Assert.assertEquals("주문 가격은 가격 * 수량이다", 20000 * orderCount ,getOrder.getTotalPrice());
        Assert.assertEquals("주문 수량만큼 재고가 줄어야 한다",95,book.getStockQuantity());
    }
